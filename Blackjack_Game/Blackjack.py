...
class Card:
    """Represents a playing card with suit, rank, and score."""
    def __init__(self, suit, rank):
        self.suit = suit
        self.rank = rank
        self.score = 10 if rank in ['K', 'Q', 'J'] else 1 if rank == 'A' else int(rank)

    def display_card(self):
        """Returns the string representation of the card with appropriate color."""
        color = Fore.RED if self.suit in [diamond, heart] else Fore.WHITE
        return f'{color}{self.suit}{Fore.WHITE}{self.rank}'
